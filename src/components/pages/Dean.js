...
const Dean = () => {
    ...
    const handleApprove = async (name) => {
        try {
            const response = await axios.put("http://localhost:5000/api/events/toapprove/3", { name });
            if (!response.ok) {
    ...
    const handleReject = async (name) => {
        try {
            const response = await axios.put("http://localhost:5000/api/events/toreject/3", { name });
            if (!response.ok) {
                throw new Error('Failed to update approval status');
            }
            getRequestData();
    ...
    return (
        <div>
            <h1 className="dashboard-heading">Dean DashBoard</h1>
                    ...
                    <table className="approval-table">
                        ...
                        <tbody>
                            {approvalRequests.map(request => (
                                request.isPending3 && request.isApproved1 && request.isApproved2 && (
