...
const Security = () => {
    ...
    const handleApprove = async (name) => {
        try {
            const response = await axios.put("http://localhost:5000/api/events/toapprove/4", { name });
            if (!response.ok) {
    ...
    const handleReject = async (name) => {
        try {
            const response = await axios.put("http://localhost:5000/api/events/toreject/4", { name });
            if (!response.ok) {
                throw new Error('Failed to update approval status');
            }
            getRequestData();
                    ...
                    <table className="approval-table">
                        ...
                        <tbody>
                            {approvalRequests.map(request => (
                                request.isLateNight && request.isPending4 && request.isApproved1 && request.isApproved2 && request.isApproved3 && (
                                ...
                                request.isLateNight && !request.isPending4 && request.isApproved1 && request.isApproved2 && request.isApproved3 && (
                                    <tr key={request.id} className="table-row">
                                        ...
                                        <td className="table-cell">
                                            ...
                                            {!request.isPending4 && !request.isApproved4 && (
